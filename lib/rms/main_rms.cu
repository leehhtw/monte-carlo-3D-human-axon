...
//        printf("before\n");
    ...
    while (1){
        ...
        xParGi[1] = floor(xi[1]); 
        xParGi[2] = floor(xi[2]);
        // aidx = (unsigned long long int)( (unsigned long long int) ( ((unsigned long long int)(NPix2) )*((unsigned long long int)(NPix3))*((unsigned long long int)(xParGi[0])) + ((unsigned long long int)(NPix3))*((unsigned long long int)(xParGi[1])) ) + (unsigned long long int)(xParGi[2]) );
        ...
        // if ( APix[ (unsigned int)(NPix2*NPix3*xParGi[0] + NPix3*xParGi[1] + xParGi[2]) ] !=0 ){ break; }
        } // while
     // printf("after\n");
    // ********** Simulate diffusion ********** //
    ...
    // ai = (int)( APix[ (unsigned int)(NPix2*NPix3*xParGi[0] + NPix3*xParGi[1] + xParGi[2]) ] );
    ...
    // ========================================================================================= //
        ...
        for(int j = 0; (j < nite) && (fstep > 0); j++){ // several attempts ? 
           ...
           // if ( (~flip0[0]) && (~flip1[0]) && (~flip0[1]) && (~flip1[1]) && (~flip0[2]) && (~flip1[2]) ) {
            ...
            // aidx = (unsigned long long int)( (unsigned long long int) ( ((unsigned long long int)(NPix2) )*((unsigned long long int)(NPix3))*((unsigned long long int)(xParGj[0])) + ((unsigned long long int)(NPix3))*((unsigned long long int)(xParGj[1])) ) + (unsigned long long int)(xParGj[2]) );    
            ...
            // aj = (int) ( APix[ (unsigned int) (NPix2*NPix3*xParGj[0] + NPix3*xParGj[1] + xParGj[2]) ] );
               
            // Checking permeability
            if(Pij[ai*Nc + aj] > 0.999999){ // same compartment: full permeability
                ai = aj; // update voxel of hitted wall
                xParGi[ii_hit] = xParGj[ii_hit]; 
                } 
            else if(Pij[ai*Nc + aj] < 0.000001){ // different compartments: no permeability 
...
double Dintra     = 2.0;          // IAS diffusion at time = 0 ms
double Dextra     = 2.0;          // EAS diffusion at time = 0 ms
double t2         = 1e20;         // T2 time 
int    mspoints   = 10;           // points per mili-second
int    initFlag   = 3;            //  1 = IAS, 2 = EAS, 3 = IAS + EAS
...
// bval[i] = (grad[i]*grad[i])*(delta[i]*delta[i])*( Delta[i] - delta[i]/3.0 ); // gamma^2 * |g| * delta^2 (Delta - delta/3) => (1/ms * 1/mT) * (1/ms * 1/mT) * (mT/ mu m) * (mT/mu m) * ms * ms * ms => ms / mu m ^ 2 
// cout << bval[i] << endl;
// cout << Delta[i] << endl;
// cout << delta[i] << endl;
// cout << bvec[i*3+0] << endl;
// cout << bvec[i*3+1] << endl;
// cout << bvec[i*3+2] << endl;
} // i 
} // pgse
else{
for(i = 0; i < Nbvec; i++) {
...
char *AcharPix  = new char[NPix1*NPix2*NPix3];
// bool *AcharPixB = new bool[NPix1*NPix2*NPix3];
thrust::host_vector<bool> AcharPixB(NPix1*NPix2*NPix3);
fread(AcharPix,sizeof(char),NPix1*NPix2*NPix3,finput);
if(initFlag == 1){  // IAS
for(i = 0; i < NPix1*NPix2*NPix3; i++){
AcharPixB[i] = (bool)(AcharPix[i]);
} // i    
}
// Adapting for simulation
if(initFlag == 2){ // EAS
for(i = 0; i < NPix1*NPix2*NPix3; i++){
AcharPixB[i] = (bool)(1 - AcharPix[i]);
} // i
} // if
if(initFlag == 3){ // IAS + EAS (Not valid using bool)
cout << "ERROR: EAS+IAS option still work in progress" << endl; 
// for(i = 0; i < NPix1*NPix2*NPix3; i++){
// AcharPixB[i] = AcharPix[i] + 1;
// } // i
} // if

// Copying raw array directly to a device_vector APix
// thrust::device_vector<bool> d_APix(AcharPixB,AcharPixB+NPix1*NPix2*NPix3); 
thrust::device_vector<bool> d_APix=AcharPixB;
...
char *initFlagS = (char*)(initFlag == 1 ? "IAS" : ( initFlag == 2 ? "EAS" : ( initFlag == 3 ? "IAS+EAS" : "NOT VALID" ) ) );
...
Pij[4] = 1.0; // (2,2)_1 or (1,1)_0
Pij[8] = 1.0; // (3,3)_1 or (2,2)_0    
...
// Some Parameters (13) of the Simulation
y = 0;
...
Out[y] = -1.0;       y++;  // 11. Permeability of a lipid bi-layer in um/ms
